<p align="center">
  <picture>
    ...
    <img src="docs/assets/brand/grc-logo-light.svg" alt="GRC 品牌 Logo" width="360" height="84">
  </picture>
</p>

# Opsy — Guided Shopify Operations

[English](README.md) · **简体中文 (ZH)**
...
客户交付包含 **Opsy Skill 目录或 GitHub 项目 + 已做好的客户项目 ZIP + 安装带教**。服务方预填[一页交付卡](docs/templates/customer-handover/客户交付卡.md)，客户按[上手指南](docs/opsy-customer-handover-sop.md)配置 Codex 或 WorkBuddy 并打开项目，无需填写多份交接表。
...
服务方建档与交接按[客户项目配置标准](skills/opsy/references/client-config-standard.md)执行：新项目有 6 个 config 文件（含问题库 CSV）、11 个工作区模板。**`buyer_faq.json` 和 `content_voice.json` 共用于商品上架与 Blog 发布**，分别提供问答证据和写作规则。独立语气文件优先，只有缺失时才兼容旧档案中的语气；无效配置不回退。只读查询：`node skills/opsy/scripts/opsy.mjs list-configs --json`；已有配置按差异审核，不覆盖客户值。
...
| 工作区 `config/store-profile.json` | 企业、买家、语言、询盘入口及授权核验元数据；兼容旧项目内嵌语气 |
| 工作区 `config/buyer_faq.json` | 问题、异议、来源和答案使用资格 |
| 工作区 `config/content_voice.json` | 商品与 Blog 共用的卖家角色、专业能力、语气、必做项及禁用表达 |
