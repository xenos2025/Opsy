...
- 2026-09-22：长期业务库统一放入 config 子目录，新增 AI 搜索政策和空问题库、只读 select-ai-prompts；按审核状态与范围选择规划问题，保留旧项目配置与证据门。

- 按商品上架与 Blog 发布整理客户配置：`buyer_faq.json` 提供问题与答案证据，新增独立 `content_voice.json` 提供共用写作规则、`validate-content-voice` 和内容上下文输出。状态、商品、Blog、批量校验与预览使用同一有效语气；独立文件优先，仅缺失时兼容内嵌旧语气，无效值不回退。语气须通过字段及就绪校验，内容上下文随语气变更失效。新项目共用 9 个工作区模板（4 个 config 文件）；保留旧项目数据，不自动迁移客户字段。只读 `list-configs --json` 同步列出实际清单。
